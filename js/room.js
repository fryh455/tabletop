...
async function rollItemInline(char, item){
  ...
  const pen=mentalPenalty(num(char.mental,0));
  const die=rollDice("normal").dice[0];
  let total=die+base+mod+pen + mm.diceBonus;
  total = applyModOp(total, num(it.mod,0), it.op);
...
async function rollAdvInline(char, adv){
  ...
  let total=die+base+mod+pen + mm.diceBonus;
  const mod = num(adv.mod,0);
  if(mod>0){ total = applyModOp(total, mod, adv.op); }
...
function ensureSheetWindow(){
  ...
  el.innerHTML = `
    <div id="swBar" style="cursor:move; display:flex; align-items:center; justify-content:space-between; gap:10px; padding:10px 12px; background:rgba(255,255,255,.04);">
      ...
      <div class="actions" style="gap:8px"><button class="danger" id="swDel" style="padding:6px 10px; display:none">Apagar</button><button class="secondary" id="swClose" style="padding:6px 10px">Fechar</button></div>
    </div>
...
async function openSheetWindow(tokenId, sx=null, sy=null){
  ...
  el.querySelector("#swDel").style.display = isMaster() ? "inline-flex" : "none";
  el.querySelector("#swEditBase").style.display = isMaster() ? "inline-flex" : "none";
  el.querySelector("#swEditBase").onclick = ()=>{ const ch=getCharByToken(sheetTokenId); if(ch) openCharBaseEditor(ch); };
