...
async function endPointer(sx,sy){
  if(!down) return;
  down=false;
  ...
  if(dist <= 6){
    const w=screenToWorld(sx,sy);
    const hit=hitToken(w.x,w.y);
    if(hit && canOpenSheet(hit.t)){
      openSheetWindow(hit.id).catch(()=>{});
    }
  }
}
...
async function openSheetWindow(tokenId){
  const t=tokens?.[tokenId];
  if(!t) return;
...
function openAdvEditor(char, idx){
  ...
  const a = (idx==null || idx<0) ? { id:`adv_${Date.now()}`, name:"Vantagem", desc:"", type:"ativa", mod:0, attrUsed:"QI", dt:9 } : { ...advs[idx] };
  const modal=document.createElement("div");
  modal.style.cssText="position:fixed; left:50%; top:50%; transform:translate(-50%,-50%); z-index:90; background:rgba(15,20,32,.97); border:1px solid rgba(255,255,255,.10); padding:14px; border-radius:16px; width:360px; max-width:92vw";
  modal.innerHTML = `
    ...
    <label class="label" style="margin-top:10px">Tipo</label>
    <input id="tp" value="${esc(a.type||"ativa")}" />
    <div class="grid2" style="margin-top:10px">
      <div>
        <label class="label">Mod. dano</label>
  ...
  modal.querySelector("#save").onclick=async ()=>{
    ...
    a.desc = clampLen(modal.querySelector("#ds").value, 180);
    a.type = clampLen(modal.querySelector("#tp").value, 30);
...
async function openCreateSheet(){
  const modal=document.createElement("div");
  modal.style.cssText="position:fixed; left:50%; top:50%; transform:translate(-50%,-50%); z-index:80; background:rgba(15,20,32,.97); border:1px solid rgba(255,255,255,.10); padding:14px; border-radius:16px; width:420px; max-width:92vw; max-height:82vh; overflow:auto";
  modal.innerHTML = `
  ...
  const renderItems=()=>{
    ...
    items.forEach((it, idx)=>{
      ...
      div.innerHTML = `<div class="actions" style="justify-content:space-between">
        <div><strong>${esc(it.name)}</strong><br/><small>${esc(it.attrUsed)} + ${it.mod} | ${it.kg}kg</small></div>
        <div class="actions"><button class="danger" data-d="${idx}">Del</button></div>
      </div>`;
  ...
  const renderAdvs=()=>{
    ...
    advs.forEach((a, idx)=>{
      ...
      div.className="item";
      div.innerHTML = `<div class="actions" style="justify-content:space-between">
        <div><strong>${esc(a.name)}</strong><br/><small>${esc(a.attrUsed)} + ${a.mod} | DT ${a.dt}</small></div>
        <div class="actions"><button class="danger" data-d="${idx}">Del</button></div>
      </div>`;
  ...
  modal.querySelector("#addIt").onclick=()=>{ items.push({ id:`it_${Date.now()}`, name:"Item", desc:"", kg:1, mod:0, attrUsed:"FOR" }); renderItems(); };
  modal.querySelector("#addAdv").onclick=()=>{ advs.push({ id:`adv_${Date.now()}`, name:"Vantagem", desc:"", type:"ativa", mod:0, attrUsed:"QI", dt:9 }); renderAdvs(); };
  renderItems(); renderAdvs();
