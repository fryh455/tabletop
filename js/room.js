...
async function openSheetPopupForToken(tokenId){
  ...
  if(!char){ toast("Esse token não tem ficha ainda.", "error"); return; }
...
/* =================== MASTER: CREATE/EDIT SHEET =================== */
let editEnabled = {}; // charId -> bool

async function openCreateSheet(){
  openModal("Criar ficha", `
    <div class="item">
  ...
  $("#create").onclick = async ()=>{
    ...
    closeModal();
    toolsState.section="sheets";
    syncToolsUI();
  };
}

async function openSheetPopupForChar(charId){
  const char = characters?.[charId];
  if(!char) return;

  const enabled = !!editEnabled[charId];
  ...
  openModal("Ficha", `
    <div class="item">
      <div class="actions" style="justify-content:space-between">
        <strong>${esc(char.name||"Ficha")}</strong>
        <button class="secondary" id="toggleEdit">${enabled?"Travado":"Atualizar"}</button>
      </div>

      <label class="label" style="margin-top:8px">Mental</label>
      <input id="m" type="number" value="${num(char.mental,0)}" ${enabled?"":"disabled"} />
      ...
      <div class="card pad" style="margin-top:10px">
        <div class="actions" style="justify-content:space-between">
          <strong>Itens</strong>
          <button class="secondary" id="addItem" ${enabled?"":"disabled"}>+ item</button>
        </div>
        <div id="inv" class="list" style="margin-top:8px"></div>
      </div>
      ...
      <div class="card pad" style="margin-top:10px">
        <div class="actions" style="justify-content:space-between">
          <strong>Vantagens</strong>
          <button class="secondary" id="addAdv" ${enabled?"":"disabled"}>+ vantagem</button>
        </div>
        <div id="advs" class="list" style="margin-top:8px"></div>
      </div>

      <div class="actions" style="margin-top:10px">
        ${enabled?`<button id="save">Salvar</button>`:""}
        <button class="secondary" id="close">Fechar</button>
      </div>
    </div>
  `);
  ...
  $("#close").onclick=closeModal;
  $("#toggleEdit").onclick = ()=>{ editEnabled[charId]=!editEnabled[charId]; closeModal(); openSheetPopupForChar(charId); };
  $$("[data-attr]").forEach(el=> el.onclick = ()=> rollAttrInline(char, el.dataset.attr));
  ...
  inv.forEach((it, idx)=>{
    ...
    div.innerHTML = `
      <div class="kv">
        ...
        ${enabled?`<button class="danger" data-del="${idx}">Del</button>`:""}
      </div>
    `;
    div.onclick = ()=> rollItemInline(char, inv[idx]);
    if(enabled){
      div.querySelector("[data-del]").onclick = async (ev)=>{
        ev.stopPropagation();
        const next = inv.filter((_,i)=>i!==idx);
        await dbUpdate(`rooms/${roomId}/characters/${charId}`, { inventory: next, updatedAt: Date.now() });
        toast("Item removido.", "ok");
        closeModal(); openSheetPopupForChar(charId);
      };
    }
    invRoot.appendChild(div);
  });
  ...
  advs.forEach((a, idx)=>{
    ...
    div.innerHTML = `
      <div class="kv">
        ...
        <div style="text-align:right">
          ...
          ${enabled?`<div style="margin-top:6px"><button class="danger" data-del="${idx}">Del</button></div>`:""}
        </div>
      </div>
    `;
    ...
    if(enabled){
      div.querySelector("[data-del]").onclick = async (ev)=>{
        ev.stopPropagation();
        const next = advs.filter((_,i)=>i!==idx);
        await dbUpdate(`rooms/${roomId}/characters/${charId}`, { advantages: next, updatedAt: Date.now() });
  ...
  if(enabled){
    $("#addItem").onclick = async ()=>{
      const next = inv.concat([{ id:`it_${Date.now()}`, name:"Item", desc:"", kg:1, mod:0, attrUsed:"FOR" }]);
      await dbUpdate(`rooms/${roomId}/characters/${charId}`, { inventory: next, updatedAt: Date.now() });
      closeModal(); openSheetPopupForChar(charId);
    };
    $("#addAdv").onclick = async ()=>{
      const next = advs.concat([{ id:`adv_${Date.now()}`, name:"Vantagem", desc:"", type:"ativa", mod:0, attrUsed:"QI", dt:9 }]);
      await dbUpdate(`rooms/${roomId}/characters/${charId}`, { advantages: next, updatedAt: Date.now() });
      closeModal(); openSheetPopupForChar(charId);
    };
    $("#save").onclick = async ()=>{
      await dbUpdate(`rooms/${roomId}/characters/${charId}`, { mental: num($("#m").value,0), updatedAt: Date.now() });
...
/* =================== MASTER TOOLS POPUP (ONE VIEW AT A TIME) =================== */
...
function ensureToolsPopup(){
  ...
  el.id="masterTools";
  el.style.cssText = `
    position:fixed; left:18px; top:86px; width:340px; max-width:92vw; z-index:50;
  ...
  el.innerHTML = `
    <div id="mtBar" style="cursor:move; display:flex; align-items:center; justify-content:space-between; gap:10px; padding:10px 12px; background:rgba(255,255,255,.04);">
      <div style="display:flex; gap:10px; align-items:center">
        ...
        <select id="mtMenu" style="height:32px">
          <option value="tokens">Tokens</option>
          <option value="sheets">Fichas</option>
          <option value="players">Players</option>
          <option value="markers">Marcos</option>
          <option value="map">Mapa/Fog</option>
          <option value="rolls">Rolagens</option>
  ...
  el.querySelector("#mtMenu").onchange = ()=>{ toolsState.section = el.querySelector("#mtMenu").value; syncToolsUI(); };
  return el;
}
...
fab.style.cssText = `
  ...
  color:#eaf2ff; font-weight:800; font-size:18px;
...
function syncToolsUI(){
  ...
  const menu = tools.querySelector("#mtMenu");
  menu.value = toolsState.section;

  const body = tools.querySelector("#mtBody");
  body.innerHTML = renderToolsSection(toolsState.section);
  bindToolsSection(toolsState.section);
}
...
function renderToolsSection(section){
    ...
    return `
      <div class="card pad">
        ...
        <div class="actions" style="margin-top:10px; flex-wrap:wrap">
          ...
          <button class="secondary" id="bgUpload">Upload p/ PostImage</button>
        </div>
      ...
      <div class="card pad" style="margin-top:10px">
        ...
        <div class="actions" style="margin-top:8px; flex-wrap:wrap">
          ...
          <button class="secondary" id="fogClear">Limpar</button>
        ...
        <small style="color:var(--muted)">Players veem fog como preto total.</small>
      </div>
    `;
  }
    ...
    return `
      <div class="card pad">
        <div class="actions" style="justify-content:space-between">
          ...
          <button class="secondary" id="createSheet">Criar ficha</button>
        </div>
        <div class="list" style="margin-top:8px">
  ...
  if(section==="players"){
    const list = Object.values(players||{}).sort((a,b)=>(a.uid||"").localeCompare(b.uid||""));
    return `
      <div class="card pad">
        ...
        <div class="list" style="margin-top:8px">
          ${list.length? list.map(p=>`
            <div class="item">
              <div class="actions" style="justify-content:space-between">
                <strong class="mono">${esc(p.uid||"")}</strong>
                <span class="badge">${esc((p.role||"player").toUpperCase())}</span>
              </div>
              <small>token: ${p.tokenId?esc(p.tokenId):"—"} | ficha: ${p.characterId?esc(p.characterId):"—"}</small>
            </div>
          `).join("") : `<div class="item"><small>Nenhum player.</small></div>`}
        </div>
      </div>
    `;
  }
        ...
        <div class="list" style="margin-top:8px">
          ${list.length? list.map(([id,m])=>`
            <div class="item">
              ...
              <small class="mono">x:${num(m.x,0).toFixed(0)} y:${num(m.y,0).toFixed(0)} | tokens: ${(m.tokenIds||[]).length}</small>
            </div>
...
function bindToolsSection(section){
  const tools = document.getElementById("masterTools");
  if(!tools) return;
  const body = tools.querySelector("#mtBody");

  if(section==="map"){
    const postKeyEl = body.querySelector("#postKey");
    if(postKeyEl) postKeyEl.onchange = ()=> localStorage.setItem("sur4_postimage_key", postKeyEl.value.trim());
  ...
  if(section==="tokens"){
    body.querySelector("#newToken").onclick = async ()=>{
    ...
    body.querySelectorAll("[data-edit-token]").forEach(b=> b.onclick = ()=> openTokenEditor(b.dataset.editToken));
    body.querySelectorAll("[data-del-token]").forEach(b=> b.onclick = async ()=>{ await dbSet(`rooms/${roomId}/tokens/${b.dataset.delToken}`, null); toast("Token deletado.","ok"); });
    return;
  }

  if(section==="sheets"){
    body.querySelector("#createSheet").onclick = openCreateSheet;
    body.querySelectorAll("[data-open-char]").forEach(b=> b.onclick = ()=> openSheetPopupForChar(b.dataset.openChar));
    return;
  }

  if(section==="markers"){
    body.querySelector("#placeMarker").onclick = ()=>{ placingMarker=true; toast("Clique no mapa para colocar o marco.","ok"); };
    body.querySelectorAll("[data-open-marker]").forEach(b=> b.onclick = ()=> openMarkerPopup(b.dataset.openMarker).catch(()=>{}));
    return;
  }
...
async function openMarkerPopup(markerId){
  ...
  openModal("Marco", `
    <div class="item">
      ...
      <div class="actions" style="margin-top:10px; flex-wrap:wrap">
        ...
        <button class="danger" id="del">Deletar</button>
      </div>
  ...
  $("#save").onclick = async ()=>{
    const patch = { title: clampLen($("#mkTitle").value, 80), tokenIds, items, updatedAt: Date.now() };
...
(function injectStyle(){
  const css = `
    #panelRoot{ padding:0 !important; height: calc(100vh - 76px); }
    .canvasWrap.full{ height:100%; }
    .canvasWrap.full canvas{ width:100%; height:100%; display:block; border-radius:0 !important; touch-action:none; }
  `;
...
onAuth(async (user)=>{
  ...
  try{
    ...
    subAll();
    resizeCanvas();
    toolsState.open = isMaster(); // master starts open
    syncToolsUI();
    mapRender();
  }catch(e){
