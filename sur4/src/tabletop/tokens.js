...
const imageCache = new Map(); // tokenId -> HTMLImageElement
...
export function renderTokens(ctx, { worldToScreen, zoom }) {
  for (const t of state.room.tokens) {
    ...
    if (state.ui.selectedTokenId === t.id) {
      ctx.save();
      ctx.strokeStyle = "rgba(91,214,255,0.8)";
...
export function hitTestToken(wx, wy, zoom) {
  // top-most = last
...
export function mountInspector() {
  ...
  function openTokenModal() {
    ...
    if (state.session.role !== "master") {
      alert("Somente o mestre cria tokens no MVP.");
      return;
    }
    inputOwner.value = "";
    inputSize.value = "64";
  ...
  function renderInspector() {
    ...
    const btnApply = inspector.querySelector("#token-apply-link");
    const btnDelete = inspector.querySelector("#token-delete");
